...
exports.main = async (event) => {
  ...
  if (!C.INTERNAL_TOKEN) {
    // 未配置环境变量时直接报错，避免空令牌被绕过
    return C.fail(C.ERR.INTERNAL, '服务端未配置 INTERNAL_TOKEN 环境变量，无法下发订阅消息');
  }
  ...
  if (!templateId) {
    console.warn(`[sendSubscribeMsg] 模板 ID 未配置（环境变量 TMPL_${scene}），跳过 ${scene} 下发`);
