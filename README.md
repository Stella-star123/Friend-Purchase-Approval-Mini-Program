...
### 3. 填配置（前端 2 处 + 云函数环境变量 4 项）

**① 前端 `miniprogram/config/index.js`（客户端发起订阅必需，属公开配置）**
```js
...
```
> `CLOUD_ENV` 和这两个模板 ID 必须填在**前端**——`wx.requestSubscribeMessage` 是客户端 API，
> 只能从前端传模板 ID，无法读云函数环境变量。它们是公开值（会下发给用户微信），不是秘密。

**② 云函数环境变量（4 项，代码里不再写死）**

打开「云开发控制台 → 云函数 → 配置 → 环境变量」，对每个云函数（或统一在环境配置）添加：

| 变量名 | 含义 | 示例 |
|--------|------|------|
| `APPROVER_OPENIDS` | 审批人 openid 白名单，逗号分隔 | `oABC123,oDEF456` |
| `TMPL_PENDING` | 待审批提醒模板 ID | `abcdefghijklmnopqrstuvwxyz123456` |
| `TMPL_RESULT` | 审批结果通知模板 ID | `zyxwvutsrqponmlkjihgfedcba654321` |
| `INTERNAL_TOKEN` | 云函数互调令牌（随机串，32 位以上） | `a8f3k…长随机串` |

> 拿审批人 openid 的最快方式：让审批人先用微信打开一次小程序，再到云开发
> 「用户」里抄他的 openid；或先随便提交一条申请，在数据库 `approvals` 的 `_openid` 字段看。
>
> ⚠️ 这 4 项**只在控制台配，不进代码**。改完保存即生效，**无需重传云函数**。
> 未配置时：审批人列表为空（所有人都是非审批人）、模板 ID 为空（自动跳过下发）、
> 令牌为空（下发直接报错），均为 fail-closed，不会出现明文泄露。

**③ 同步共享模块（仅在改了 `shared/common.js` 的逻辑时才需要）**
```bash
node scripts/sync-shared.js
```
> 云函数不能跨目录 require，`common.js` 是分发到 6 个函数目录的副本。
> 现在配置文件里已无密钥，但模块本身的代码更新仍需同步。脚本跑完会提示去控制台配环境变量。
...
> 字段 key 不一致会报 **47003**；模板 ID 没配时（环境变量为空）代码会自动跳过下发，不会报错。
...
node scripts/test-cloudfunctions.js # 逻辑测试：74 项断言，不需要部署
```
